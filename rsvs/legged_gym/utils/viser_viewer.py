...
class ViserViewer:
    ...
    def _build_scene(self) -> None:
        ...
        self._command_sliders: Dict[str, object] = {}
        self._gait_dropdown = None
        self._heading_command = False
        self._has_wtw_controls = False
    ...
    def _add_range_slider(
        ...
        """Create a slider, clamping initial value into [min, max]."""
        lo = float(value_range[0])
        hi = float(value_range[1])
        if lo > hi:
            lo, hi = hi, lo
        initial = float(np.clip(initial_value, lo, hi))
    ...
    def setup_env_controls(self, env) -> None:
        """Build GUI controls from the environment/task configuration."""
        cfg = env.cfg
        cmd_ranges = cfg.commands.ranges
        self._heading_command = getattr(cfg.commands, "heading_command", False)

        with self.server.gui.add_folder("Velocity Commands", expand_by_default=True):
            self._add_range_slider(
                "lin_vel_x",
                "Linear X (m/s)",
                cmd_ranges.lin_vel_x,
                env.commands[0, 0].item(),
                step=0.05,
            )
            self._add_range_slider(
                "lin_vel_y",
                "Linear Y (m/s)",
                cmd_ranges.lin_vel_y,
            ...
            if self._heading_command:
                ...
                self._add_range_slider(
                    "ang_vel_yaw",
                    "Angular Yaw (rad/s)",
                    cmd_ranges.ang_vel_yaw,
                    env.commands[0, 2].item(),
                    step=0.05,
                )

        if hasattr(env, "gait_period"):
            self._setup_wtw_behavior_sliders(env)

    def _setup_wtw_behavior_sliders(self, env) -> None:
        """Add WTW-specific behavior parameter controls."""
        behavior_cfg = env.cfg.rewards.behavior_params_range
        self._has_wtw_controls = True

        with self.server.gui.add_folder("WTW Behavior", expand_by_default=True):
            self._gait_dropdown = self.server.gui.add_dropdown(
                "Gait type",
                options=tuple(self.GAIT_PRESETS.keys()),
                initial_value="trot",
            )
            self._add_range_slider(
                "gait_period",
                "Gait period (s)",
                behavior_cfg.gait_period_range,
                env.gait_period[0, 0].item(),
                step=0.01,
            )
            self._add_range_slider(
                "base_height_target",
                "Base height target (m)",
                behavior_cfg.base_height_target_range,
                env.base_height_target[0, 0].item(),
                step=0.01,
            )
            self._add_range_slider(
                "foot_clearance_target",
                "Foot clearance target (m)",
                behavior_cfg.foot_clearance_target_range,
                env.foot_clearance_target[0, 0].item(),
                step=0.01,
            )
            self._add_range_slider(
                "pitch_target",
                "Pitch target (rad)",
                behavior_cfg.pitch_target_range,
                env.pitch_target[0, 0].item(),
                step=0.05,
            )

    def apply_commands_to_env(self, env) -> None:
        """Write slider values into the environment command/behavior buffers."""
        if not self._command_sliders:
            return

        env.commands[:, 0] = self._command_sliders["lin_vel_x"].value
        env.commands[:, 1] = self._command_sliders["lin_vel_y"].value
        if self._heading_command:
            env.commands[:, 3] = self._command_sliders["heading"].value
        else:
            env.commands[:, 2] = self._command_sliders["ang_vel_yaw"].value

        if not self._has_wtw_controls:
            return

        env.gait_period[:] = self._command_sliders["gait_period"].value
        env.base_height_target[:] = self._command_sliders["base_height_target"].value
        env.foot_clearance_target[:] = self._command_sliders["foot_clearance_target"].value
        env.pitch_target[:] = self._command_sliders["pitch_target"].value

        if self._gait_dropdown is not None:
            theta = self.GAIT_PRESETS[self._gait_dropdown.value]
            env.theta[:, 0] = theta[0]
            env.theta[:, 1] = theta[1]
            env.theta[:, 2] = theta[2]
            env.theta[:, 3] = theta[3]
    ...
    def get_command(self) -> np.ndarray:
        ...
        Returns:
            np.ndarray: [lin_vel_x, lin_vel_y, ang_vel_yaw_or_heading]
        """
        if not self._command_sliders:
            return np.array([0.0, 0.0, 0.0])

        yaw_or_heading = (
            self._command_sliders["heading"].value
            if self._heading_command
            else self._command_sliders["ang_vel_yaw"].value
        )
        return np.array([
...
def create_viser_viewer(env, port: int = 8080, robot_index: int = 0) -> ViserViewer:
    ...
    try:
        ...
        if mesh_type == 'plane':
            ...
            terrain_mesh = None
            gs_terrain = getattr(getattr(env.simulator, '_gs_terrain', None), 'geoms', None)
            if gs_terrain:
                # Use Genesis collision mesh so Viser matches physics (heightfield rebuild can misalign).
                import copy
                terrain_mesh = copy.deepcopy(gs_terrain[0].get_trimesh())
                transform = np.array([
                    -env.cfg.terrain.border_size,
                    -env.cfg.terrain.border_size,
                    0.0,
                ])
                terrain_mesh.apply_transform(
                    trimesh.transformations.translation_matrix(transform)
                )
            elif hasattr(terrain, 'heightsamples'):
                transform = np.zeros((3,))
                transform[0] = -env.cfg.terrain.border_size
                transform[1] = -env.cfg.terrain.border_size
                transform[2] = 0.0
                terrain_mesh = viewer.set_terrain_from_heightfield(
