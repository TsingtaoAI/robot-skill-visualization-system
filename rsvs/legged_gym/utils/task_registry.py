...
class TaskRegistry():
    ...
    def make_alg_runner(self, env, name=None, args=None, train_cfg=None, log_root="default"):
        ...
        if resume:
            ...
            # resolve load_run/checkpoint for downstream export and logging
            train_cfg.runner.load_run = os.path.basename(os.path.dirname(resume_path))
            if train_cfg.runner.checkpoint == -1:
                model_name = os.path.basename(resume_path)
                train_cfg.runner.checkpoint = int(model_name.split('_')[1].split('.')[0])
